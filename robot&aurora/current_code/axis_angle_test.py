...
r1 = R.from_rotvec(np.deg2rad([0, -64.52, 168.04]))
r1_quaternion = r1.as_quat()
r_vec1 = r1.as_rotvec(degrees=True)
r2 = R.from_rotvec(np.deg2rad([0, 64.52, -168.04]))
...
q3 = normalize_quat_sign([0.0000, -0.3584, 0.9336, -0.00000000000000000000000000000000000000000000000001])
r3 = R.from_quat(q3)
r3_vec = r3.as_rotvec(degrees=True)

q4 = normalize_quat_sign([0.0000, -0.3584, 0.9336, 0.0000000000000000000000000000000000000000000000000001])
r4 = R.from_quat(q4)
r4_vec = r4.as_rotvec(degrees=True)
