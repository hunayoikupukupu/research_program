...
# 変換行列を求める
R2_all, t2_all = findTransformation(P1_all, P2_all)
...
for part in range(split):
    ...
    # 変換行列を求める
    R2, t2 = findTransformation(P1, P2)
...
while True:
    ...
    P_target.append([probes[0].pos.x, probes[0].pos.y, probes[0].pos.z])
    
    P_translation = np.array(P_target) - np.array(t2_all)
    P_next = np.dot(P_translation, R2_all)
    print(P_next)

    arm.set_position(x=P_next[0][0] +75, y=P_next[0][1] + 10, z=P_next[0][2] +25, wait=True)
    ...
    P_robot.append([robot.pos.x, robot.pos.y, robot.pos.z])
    P_aurora.append([probes[1].pos.x, probes[1].pos.y, probes[1].pos.z])
