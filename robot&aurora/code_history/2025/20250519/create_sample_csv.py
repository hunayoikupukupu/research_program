...
def generate_synthetic_data(
    ...
    with open(output_file, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['robot_x', 'robot_y', 'robot_z', 'aurora_x', 'aurora_y', 'aurora_z'])
...
if __name__ == "__main__":
    ...
    result = main(
        ...
        rotation_euler_deg=[56, -31, -88],         # 回転角度（オイラー角XYZ、度数法）
        translation_vector=[-20, 44, 99],      # 並進ベクトル [tx, ty, tz]
        step_size=10,                          # サンプリング間隔（mm）
        # num_points_per_axis=5,               # 各軸の点の数（指定する場合はstep_sizeをコメントアウト）
        output_file="robot&aurora/current_code/calibration_data/aurora_robot_pose_log_r56_p-31_y-88_static.csv",  # 出力CSVファイル名
