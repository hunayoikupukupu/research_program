...
for i in range(7):# xを動かすfor文
    for j in range(10):# yを動かすfor文
        for k in range(11):# zを動かすfor文
            arm.set_position(x=200+10*i, y=-110+20*j, z=230-20*k, wait=True)
            time.sleep(2)
            #robot座標系中のセンサー位置を出力
...
#CSVファイルに出力
with open("robot&aurora/20240228/data_infront_0228.csv", "w", newline="") as f:
    writer = csv.writer(f)
